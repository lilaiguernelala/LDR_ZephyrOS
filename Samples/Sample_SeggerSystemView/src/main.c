...
{
    //if (k_mutex_lock(&mutexLEDs, K_FOREVER) == 0) // Enlever pour ne pas générer d'overflow dans SeggerSystemView
    {
...
static void led0_task(void *p1, void *p2, void *p3)
{
    ...
    while (1)
    {
        LOG_INF("LED0 start at %d", k_cycle_get_32());
        for (i = 0; i < nbIter; i++)
        {
            update_leds(1, 0);
        }
        update_leds(0, 0);
        k_msleep(1000);
        LOG_INF("LED0 end at %d", k_cycle_get_32());
    }
}
...
static void led1_task(void *p1, void *p2, void *p3)
{
    ...
    while (1)
    {
        ...
        update_leds(0, 0);
        k_msleep(2000);
        LOG_INF("LED1 task at %d", k_cycle_get_32());
    }
...
static void sw0_task(void *p1, void *p2, void *p3)
{
    ...
    while (1)
    {
        if (switchPushed == 1)
        {
            ...
            update_leds(0, 0);
            LOG_INF("SW0 task at %d", k_cycle_get_32());
            printk("SW0 task at %d\n", k_cycle_get_32());
        }
        k_yield();
    }
}
...
void main(void)
{
    ...
    k_tid_t tidLed0 = k_thread_create(&led0_t, led0_stack, K_THREAD_STACK_SIZEOF(led0_stack),
                    led0_task, NULL, NULL, NULL,
                    1, 0, K_NO_WAIT);
    k_thread_name_set(tidLed0, "led0");

    k_tid_t tidLed1 = k_thread_create(&led1_t, led1_stack, K_THREAD_STACK_SIZEOF(led1_stack),
                    led1_task, NULL, NULL, NULL,
                    2, 0, K_NO_WAIT);
    k_thread_name_set(tidLed1, "led1");

    k_tid_t tidSw0 = k_thread_create(&sw0_t, sw0_stack, K_THREAD_STACK_SIZEOF(sw0_stack),
                    sw0_task, NULL, NULL, NULL,
                    3, 0, K_NO_WAIT);
    k_thread_name_set(tidSw0, "sw0");
}