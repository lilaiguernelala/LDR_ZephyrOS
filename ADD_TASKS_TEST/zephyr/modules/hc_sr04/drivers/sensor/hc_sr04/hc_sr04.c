...
static const struct hc_sr04_config hc_sr04_config = {
	.trig_pin = DT_INST_GPIO_PIN(0, trig_gpios),
	.trig_ctrl = DEVICE_DT_GET(DT_GPIO_CTLR(DT_DRV_INST(0), trig_gpios)),
	.trig_flags = DT_INST_GPIO_FLAGS(0, trig_gpios),

	.echo_pin = DT_INST_GPIO_PIN(0, echo_gpios),
	.echo_ctrl = DEVICE_DT_GET(DT_GPIO_CTLR(DT_DRV_INST(0), echo_gpios)),
	.echo_flags = DT_INST_GPIO_FLAGS(0, echo_gpios),
};
...
static void hc_sr04_gpio_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	//const struct hc_sr04_config *p_cfg = p_data->dev->config;
...
DEVICE_DT_INST_DEFINE(0, hc_sr04_init, NULL, &hc_sr04_data, &hc_sr04_config, POST_KERNEL,
